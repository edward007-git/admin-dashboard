    ...
    <div className="w-full grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {stats.map((stat, index) => (
        <div
         ...
         border-slate-200/50 dark:border-slate-700/50 rounded-xl p-4 sm:p-5 hover:shadow-xl 
         hover:shadow-slate-200/20 dark:hover:shadow-slate-900/20 transition-all duration-300"
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1.5 truncate">
                {stat.title}
              </p>

              <p className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white mb-3">
                {stat.value}
              </p>
            ...
            {/* Icon Box */}
            <div
              className={`p-2.5 sm:p-3 rounded-xl ${stat.bgColor} transition-all duration-200 flex-shrink-0`}
            >
          ...
          {/* Progress Bar */}
          <div className="mt-3 sm:mt-4 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
